...
if ! command -v x-ui &>/dev/null; then
  echo "⏳ ไม่พบ x-ui กำลังดาวน์โหลดและติดตั้งเวอร์ชัน v2.9.4..."
  
  # 1. สร้างไฟล์สคริปต์ชั่วคราว
  local _xui_sh; _xui_sh=$(mktemp /tmp/xui-XXXXX.sh)
  
  # 2. ดาวน์โหลดสคริปต์ติดตั้งหลักจากคลังของ MHSanaei
  curl -Ls "https://raw.githubusercontent.com/MHSanaei/3x-ui/master/install.sh" -o "$_xui_sh" 2>/dev/null

  # 3. [แก้ไข] ใช้ sed เข้าไปเขียนทับตัวแปรเพื่อบังคับล็อกเวอร์ชันเป็น v2.9.4
  if [[ -f "$_xui_sh" ]]; then
      sed -i 's|LAST_RELEASE_VERSION=.*|LAST_RELEASE_VERSION="v2.9.4"|g' "$_xui_sh"
      sed -i 's|VERSION=.*|VERSION="v2.9.4"|g' "$_xui_sh"
  fi

  # 4. [แก้ไข] สั่งรันพร้อมส่งค่าตอบคำถามอัตโนมัติเรียงตามลำดับโครงสร้างของ v2.9.4:
  # y          -> ยืนยันการติดตั้งต่อ
  # $XUI_USER  -> ดึงค่าจากตัวแปรของคุณมาตั้งเป็น Username
  # $XUI_PASS  -> ดึงค่าจากตัวแปรของคุณมาตั้งเป็น Password
  # 2053       -> พอร์ตสำหรับหน้า Dashboard ล็อกไว้ตามมาตรฐานเดิมของคุณ
  printf "y\n%s\n%s\n2053\n" "$XUI_USER" "$XUI_PASS" | bash "$_xui_sh" >> /var/log/chaiya-xui-install.log 2>&1
  
  # 5. ลบไฟล์ชั่วคราวออกเมื่อเสร็จงาน
  rm -f "$_xui_sh"
  
  echo "✅ ติดตั้ง 3x-ui v2.9.4 สำเร็จ!"
fi
